function _defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }

define(['jsx!widgets/Unit-Input-v2', 'jsx!contexts/DialogCaller', 'app/contexts/AlertCaller', 'app/constants/alert-constants', 'app/actions/beambox/constant', 'helpers/image-data', 'helpers/i18n'], function (UnitInput, DialogCaller, Alert, AlertConstants, Constant, ImageData, i18n) {
  const React = require('react');

  const classNames = require('classnames');

  const LANG = i18n.lang.beambox.right_panel.object_panel.option_panel;

  class ImageOptions extends React.Component {
    constructor(props) {
      super(props);

      _defineProperty(this, "updateImage", () => {
        const {
          elem
        } = this.props;
        const isShading = elem.getAttribute('data-shading') === 'true';
        const threshold = parseInt(elem.getAttribute('data-threshold'));
        ImageData($(elem).attr("origImage"), {
          height: $(elem).height(),
          width: $(elem).width(),
          grayscale: {
            is_rgba: true,
            is_shading: isShading,
            threshold: threshold,
            is_svg: false
          },
          onComplete: function (result) {
            $(elem).attr('xlink:href', result.canvas.toDataURL('image/png'));
          }
        });
      });

      _defineProperty(this, "handleGradientClick", () => {
        const {
          elem,
          updateObjectPanel
        } = this.props;
        const isGradient = elem.getAttribute('data-shading') === 'true';
        const isTurningOnGradient = !isGradient;
        elem.setAttribute('data-shading', isTurningOnGradient);
        elem.setAttribute('data-threshold', isTurningOnGradient ? 254 : 128);
        this.setState(this.state);
        this.updateImage();
        updateObjectPanel();
      });

      _defineProperty(this, "handleThresholdChange", val => {
        const {
          elem
        } = this.props;
        elem.setAttribute('data-threshold', val);
        this.setState(this.state);
        this.updateImage();
      });

      this.state = {};
    }

    componentDidMount() {}

    componentWillUnmount() {}

    renderGradientBlock() {
      const {
        elem
      } = this.props;
      const isGradient = elem.getAttribute('data-shading') === 'true';
      return /*#__PURE__*/React.createElement("div", {
        className: "option-block",
        key: "gradient"
      }, /*#__PURE__*/React.createElement("div", {
        className: "label"
      }, LANG.shading), /*#__PURE__*/React.createElement("div", {
        className: "onoffswitch",
        onClick: () => this.handleGradientClick()
      }, /*#__PURE__*/React.createElement("input", {
        type: "checkbox",
        className: "onoffswitch-checkbox",
        checked: isGradient,
        readOnly: true
      }), /*#__PURE__*/React.createElement("label", {
        className: "onoffswitch-label"
      }, /*#__PURE__*/React.createElement("span", {
        className: "onoffswitch-inner"
      }), /*#__PURE__*/React.createElement("span", {
        className: "onoffswitch-switch"
      }))));
    }

    renderThresholdBlock() {
      const {
        elem
      } = this.props;
      const isGradient = elem.getAttribute('data-shading') === 'true';

      if (isGradient) {
        return null;
      }

      const threshold = parseInt(elem.getAttribute('data-threshold')) || 128;
      return /*#__PURE__*/React.createElement("div", {
        key: "threshold"
      }, /*#__PURE__*/React.createElement("div", {
        className: "option-block with-slider"
      }, /*#__PURE__*/React.createElement("div", {
        className: "label"
      }, LANG.threshold), /*#__PURE__*/React.createElement(UnitInput, {
        min: 1,
        max: 255,
        decimal: 0,
        unit: '',
        className: {
          'option-input': true
        },
        defaultValue: threshold,
        getValue: val => this.handleThresholdChange(val)
      })), /*#__PURE__*/React.createElement("div", {
        className: "option-block slider-container"
      }, /*#__PURE__*/React.createElement("input", {
        className: "threshold-slider",
        type: "range",
        min: 1,
        max: 255,
        step: 1,
        value: threshold,
        onChange: e => {
          this.handleThresholdChange(parseInt(e.target.value));
        }
      })));
    }

    render() {
      return /*#__PURE__*/React.createElement("div", null, this.renderGradientBlock(), this.renderThresholdBlock());
    }

  }

  return ImageOptions;
});