...
define(['jquery', 'reactPropTypes', 'app/actions/beambox', 'app/actions/beambox/preview-mode-background-drawer', 'app/actions/beambox/svgeditor-function-wrapper', 'app/stores/beambox-store', 'helpers/i18n', 'helpers/image-data', 'helpers/api/image-tracer', 'jsx!widgets/Modal', 'jsx!widgets/Slider-Control', 'lib/cropper', 'lib/svgeditor/imagetracer'], function ($, PropTypes, BeamboxActions, PreviewModeBackgroundDrawer, FnWrapper, BeamboxStore, i18n, ImageData, ImageTracerApi, Modal, SliderControl, Cropper, ImageTracer) {
  const React = require('react');

  const LANG = i18n.lang.beambox.image_trace_panel;
  const imageTracerWebSocket = ImageTracerApi();
  const TESTING_IT = false; //View render the following steps
  ...
  class ImageTracePanel extends React.Component {
    ...
    next() {
      switch (this.state.currentStep) {
        case STEP_NONE:
          this.setState({
            currentStep: STEP_OPEN
          });
          break;

        case STEP_OPEN:
          this.setState({
            currentStep: STEP_CROP
          });
          break;
    ...
    async _pushImageTrace() {
      ...
      if (TESTING_IT) {
        ...
        FnWrapper.insertImage(croppedBlobUrl, testingCropData, testingPreCrop, 1, threshold, true); //FnWrapper.insertSvg(imageTrace, 'image-trace', testingCropData, testingPreCrop);
      } else {
    ...
    _cropCameraCanvas() {
      ...
      canvas.toBlob(blob => {
        ...
        this.setState({
          croppedCameraCanvasBlobUrl: url
        });
    ...
    _renderImageTraceModal() {
      ...
      return /*#__PURE__*/React.createElement(Modal, null, /*#__PURE__*/React.createElement("div", {
        ...
        className: "title"
      }, LANG.tuning), /*#__PURE__*/React.createElement(SliderControl, {
        id: "threshold",
        key: "threshold",
    ...
    _renderContent() {
      ...
      switch (this.state.currentStep) {
        case STEP_OPEN:
          renderContent = /*#__PURE__*/React.createElement("img", {
            id: "cameraCanvas",
