...
export default class CurveControl extends React.PureComponent {
  constructor(props) {
    super(props);
    const { updateCurveFunction } = this.props;
    this.state = {
  ...
  cubicSplinesInterpolation = (x: number): number => {
    const { controlPoints, splineKs } = this.state;
  ...
  addControlPoint = (e): void => {
    const { controlPoints, draggingIndex } = this.state;
    if (typeof draggingIndex === 'number') {
      return;
    }
