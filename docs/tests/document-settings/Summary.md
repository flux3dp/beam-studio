...
共用基礎：`apps/web/cypress/support/taskPath.ts`（gcode 擷取／解析／度量／golden 快照模組）與 `.agents/skills/e2e-test/SKILL.md` 的「Path-based verification」準則。其餘項目（當前位置雕刻、實機雕刻品質）維持實機人工。
