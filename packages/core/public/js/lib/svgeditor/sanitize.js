...
(function () {
  ...
  svgedit.sanitize.sanitizeSvg = function (node) {
    // Cleanup text nodes
    if (node.nodeType == 3 && node.parentNode && node.parentNode.tagName !== 'tspan') {
      // 3 == TEXT_NODE
      // Trim whitespace
    ...
    if (typeof allowedAttrs !== 'undefined') {
      ...
      if (
        href &&
        ['filter', 'linearGradient', 'pattern', 'radialGradient', 'textPath', 'use'].indexOf(node.nodeName) >= 0
      ) {
