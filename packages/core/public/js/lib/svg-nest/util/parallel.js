(function (root) {
    ...
    function SvgNestParallel(data, options) {
        this.data = data;
    ...
    // static method
    SvgNestParallel.isSupported = function () { return _supports; }

    SvgNestParallel.prototype.getWorkerSource = function (cb, env) {
        var that = this;
        var preStr = '';
        var i = 0;
    ...
    SvgNestParallel.prototype.spawn = function (cb, env) {
        var that = this;
    ...
    SvgNestParallel.prototype._spawnMapWorker = function (i, cb, done, env, wrk) {
        var that = this;
    ...
    SvgNestParallel.prototype.map = function (cb, env) {
        env = extend(this.options.env, env || {});
        if (!this.data.length) {
    ...
    SvgNestParallel.prototype._spawnReduceWorker = function (data, cb, done, env, wrk) {
        var that = this;
        if (!wrk) wrk = that._spawnWorker(cb, env);
    ...
    SvgNestParallel.prototype.reduce = function (cb, env) {
        env = extend(this.options.env, env || {});
    ...
    SvgNestParallel.prototype.then = function (cb, errCb) {
        var that = this;
        var newOp = new Operation();
    ...
    root.SvgNestParallel = SvgNestParallel;
