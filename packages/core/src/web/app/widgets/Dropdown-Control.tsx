...
class DropDownControl extends React.Component<Props, State> {
  ...
  render() {
    let _options = [];
    const { hiddenOptions, id, label, options, value } = this.props;

    if (hiddenOptions) {
      hiddenOptions.forEach((option) => {
        if (typeof option === 'object') {
          _options.push(
            <option disabled hidden={option.value !== value} key={option.value} value={option.value}>
              {option.label}
            </option>,
          );
        } else {
          _options.push(
            <option disabled hidden={option !== value} key={option} value={option}>
              {option}
            </option>,
          );
        }
    ...
    const className = classNames('dropdown-container', {
      'first-child-selected': firstChildSelected,
      'more-than-one': options.length > 1,
    });
