...
const lang: ILang = {
  ...
  beambox: {
    ...
    popup: {
      ...
      too_fast_for_curve: '對包含3D曲線物件的圖層使用過高速度可能會降低雕刻精度。\n建議將速度保持在%(limit)s以下，以獲得3D曲線物件的最佳效果。',
      too_fast_for_curve_and_constrain: '以下圖層: %(layers)s 超過了速度限制 %(limit)s。\n3D曲線物件的雕刻速度將被限制為 %(limit)s。您可以在偏好設定中移除此限制。',
    ...
    right_panel: {
      laser_panel: {
        ...
        color_strength: '強度',
        cross_hatch: '交叉填充',
        curve_engraving_speed_contrain_warning: '3D 曲面物件的雕刻速度將被限制在 %(limit)s。您可以在偏好設定解除此限制。',
        custom_preset: '自訂',
        cut: '切割',
        diode: '二極體雷射',
