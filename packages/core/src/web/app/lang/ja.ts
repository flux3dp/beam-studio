...
const lang: ILang = {
  ...
  beambox: {
    ...
    popup: {
      ...
      too_fast_for_curve: '3D曲線オブジェクトを含むレイヤーに対して高すぎる速度を使用すると、彫刻の精度が低下する可能性があります。\n3D曲線オブジェクトで最適な結果を得るには、速度を%(limit)s未満に保つことをお勧めします。',
      too_fast_for_curve_and_constrain: '以下のレイヤー: %(layers)s は速度制限%(limit)sを超えています。\n3D曲線オブジェクトの彫刻速度は%(limit)sに制限されます。この制限は「設定」で解除できます。',
    ...
    right_panel: {
      laser_panel: {
        ...
        color_strength: '強度',
        cross_hatch: '交差ハッチ',
        curve_engraving_speed_contrain_warning: '3D曲線オブジェクトの彫刻速度は%(limit)sに制限されます。この制限は「好み」で解除できます。',
        custom_preset: 'カスタム',
        cut: 'カット',
        diode: 'ダイオードレーザー',
