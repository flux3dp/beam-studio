...
const lang: ILang = {
  ...
  beambox: {
    ...
    popup: {
      ...
      too_fast_for_curve: '对包含3D曲线对象的图层使用过高速度可能会降低雕刻精度。\n建议将速度保持在%(limit)s以下，以获得3D曲线对象的最佳效果。',
      too_fast_for_curve_and_constrain: '以下图层: %(layers)s 超过了速度限制 %(limit)s。\n3D曲线对象的雕刻速度将被限制为 %(limit)s。您可以在首选项设置中移除此限制。',
    ...
    right_panel: {
      laser_panel: {
        ...
        color_strength: '强度',
        cross_hatch: '交叉填充',
        curve_engraving_speed_contrain_warning: '3D 曲线对象的雕刻速度将被限制为 %(limit)s。您可以在首选项设置中删除此限制。',
        custom_preset: '自订',
        cut: '切割',
        diode: '二极体雷射',
