...
const lang: ILang = {
  ...
  beambox: {
    ...
    popup: {
      ...
      too_fast_for_curve: 'Sử dụng tốc độ quá cao cho các lớp chứa đối tượng đường cong 3D có thể làm giảm độ chính xác khắc.\nChúng tôi khuyến nghị giữ tốc độ dưới %(limit)s để đạt kết quả tối ưu với các đối tượng đường cong 3D.',
      too_fast_for_curve_and_constrain: 'Các lớp sau: %(layers)s vượt quá giới hạn tốc độ %(limit)s.\nTốc độ khắc cho các đối tượng đường cong 3D sẽ bị giới hạn ở %(limit)s. Bạn có thể loại bỏ giới hạn này trong Cài đặt Tùy chọn.',
    ...
    right_panel: {
      laser_panel: {
        ...
        curve_engraving_speed_contrain_warning: 'Tốc độ khắc của các đối tượng đường cong 3D sẽ bị giới hạn ở mức %(limit)s. Bạn có thể loại bỏ giới hạn này trong Cài đặt Tùy chọn.',
