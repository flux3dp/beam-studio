...
import { chooseCurrentDimensions } from './utils/chooseCurrentDimensions';
...
const onResizeMouseMove = (evt: MouseEvent, selected: SVGElement, x: number, y: number) => {
  ...
  if (!evt.shiftKey && svgCanvas.isBezierPathAlignToEdge) {
    const { byX, byY } = svgCanvas.findMatchPoint(x, y);
...
const mouseMove = (evt: MouseEvent) => {
  ...
  if (evt.shiftKey) removeAlignLines();

  if (!started) {
    if (svgCanvas.isBezierPathAlignToEdge && currentMode === 'path') {
      const { byX, byY } = svgCanvas.findMatchPoint(realX, realY);
  ...
  switch (currentMode) {
    case 'select':
      ...
      if (selectedElements[0] !== null) {
        ...
        if (evt.shiftKey) {
          ...
          const dim = chooseCurrentDimensions(currentBoundingBox, dx, dy);

          // const { byX, byY } = svgCanvas.findMatchPoint(x, y);

          dx = dim.x ? dim.x - startX : dx;
          dy = dim.y ? dim.y - startY : dy;

          currentBoundingBox.forEach((point, index) => {
            svgCanvas.drawTracingLine(point.x, point.y, point.x + dx, point.y + dy, index);
          });
          // svgCanvas.drawAlignLine(x, y, byX, byY);
        }
    ...
    case 'line':
      ...
      if (evt.shiftKey) {
        ...
        const { byX, byY } = svgCanvas.findMatchPoint(x2, y2);
    ...
    case 'image':
      ...
      if (!isSquare && svgCanvas.isBezierPathAlignToEdge) {
        const { byX, byY } = svgCanvas.findMatchPoint(newX, newY);
