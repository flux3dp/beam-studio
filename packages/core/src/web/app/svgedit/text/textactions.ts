...
class TextActions {
  ...
  setInputValueFromCurtext() {
    ...
    const children = Array.from(curtext.childNodes).filter((child): child is Element =>
      ['textPath', 'tspan'].includes(child.nodeName),
    );
    ...
    if (curtext.getAttribute('data-fit-text') === 'true') {
      ...
      this.textinput.value = manualLines.join('\u0085');
    } else {
      this.textinput.value = children.map((child) => child.textContent).join('\u0085');
    }
  }
