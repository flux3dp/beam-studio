...
const PATH_D_COMMAND_COUNT_THRESHOLD = 100_000; // 100,000 commands
...
async function performSvgPreChecks(
  ...
  try {
    ...
    if (file.size > sizeThresholdMaxBytes) {
      ...
      warningMessages.push(`File size is ${currentSizeMB}MB (threshold: ${maxSizeMB}MB).`);
    }
    ...
    if (elementCount > elementCountThreshold) {
      warningMessages.push(`Contains approximately ${elementCount} elements (threshold: ${elementCountThreshold}).`);
    }
    ...
    if (isTooManyCommands) {
      warningMessages.push(
        `${totalCommands} drawing commands of paths was found (threshold: ${pathDCommandCountThreshold}).`,
      );
    }
    ...
    if (warningMessages.length > 0) {
      ...
      const adviceMessage = 'Importing may cause slowdowns or instability. Do you want to continue?';
