...
const Preview = ({ dimensions, onViewModeChange, state, typeConfig, viewMode }: PreviewProps): React.JSX.Element => {
  ...
  const renderScene = (mode: ViewMode, sceneColors: typeof colors, layout: typeof viewLayout) =>
    mode === 'design' ? (
      <DesignScene clipFunc={boundaryClip} colors={sceneColors} geometry={geometry} imageOverlay={imageOverlayNode} />
    ) : (
      <ExplodedScene
  ...
  const legendEntries = useMemo(() => {
    ...
    const entries: Array<{ color: string; label: string }> = [
      { color: COLORS.exploded.raisedEdges, label: t.raised_edges },
      { color: COLORS.exploded.pieces, label: t.puzzle_pieces },
    ];

    if (state.border.enabled) entries.push({ color: COLORS.exploded.boardBase, label: t.board_base });
