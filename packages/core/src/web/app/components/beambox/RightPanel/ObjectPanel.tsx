...
import ConfigPanel from '@core/app/components/beambox/RightPanel/ConfigPanel/ConfigPanel';
import ColorsPanel from '@core/app/components/beambox/RightPanel/ObjectPanel/ColorsPanel';
import LibraryPanel from '@core/app/components/beambox/RightPanel/ObjectPanel/LibraryPanel';
  ...
  parameter: {
    render: (key) => (
      <ObjectPanelItem
        ...
        id="parameter"
        key={key}
        renderContent={() => <ConfigPanel UIType="modal" />}
...
function ObjectPanel({ hide }: Props): React.JSX.Element {
  ...
  const renderDesktopCollapse = (): React.JSX.Element => {
    const desktopPanels: Array<null | PanelType> = [
      ...
      // templateConfig
      'transform',
      infillPanels?.length ? 'infill' : colorPanels?.length ? 'colors' : null,
    ...
    const desktopItems: CollapseProps['items'] = desktopPanels
      ...
      .map((key) => {
        const { labelKey, render } = panels[key];

        return {
          children: render(key, elem!),
          forceRender: true,
          key,
          label: labelKey ? tSections[labelKey] : key,
        };
      });
  ...
  const renderTabletButtons = () => {
    const tabletPanels: Array<'divider' | null | PanelType> = [
      ...
      'parameter',
      'divider',
      isProjectMode ? 'templateConfig' : null,
