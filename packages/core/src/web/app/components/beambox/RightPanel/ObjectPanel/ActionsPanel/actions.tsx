...
import { ArrayPopup, showArrayModal } from '@core/app/components/dialogs/ArrayModal';
import { showCurvePanel, showRotaryWarped, showSharpenPanel } from '@core/app/components/dialogs/image';
import { OffsetPopup, showOffsetModal } from '@core/app/components/dialogs/OffsetModal';
...
export interface PanelAction {
  ...
  /** @deprecated Old translations for short labels. Use label instead */
  mobileLabel?: ResolveILangFn;
  onClick: (elem: SVGElement) => void;
  /**
   * When set, the action renders its UI inline in ObjectPanelItem's popover (anchored to the
   * button, open state bound to `activeKey`) instead of opening a standalone dialog on click.
   */
  renderContent?: (props: {
...
export const actions: { [key in ActionKey]: PanelAction } = {
  array: {
    ...
    renderContent: ({ objectPanelKey, onClose, reference }) => (
      <ArrayPopup objectPanelKey={objectPanelKey} onClose={onClose} reference={reference} />
    ),
  },
  autoFit: {
  ...
  offset: {
    ...
    renderContent: ({ objectPanelKey, onClose, reference }) => (
      <OffsetPopup objectPanelKey={objectPanelKey} onClose={onClose} reference={reference} />
    ),
  },
  potrace: {
  ...
  potraceAndOffset: {
    icon: <ActionPanelIcons.BitmapOffset />,
    id: 'potrace_and_offset',
    label: (_i18n, tActionsPanel) => tActionsPanel.bitmap_offset,
    onClick: async (elem) => {
