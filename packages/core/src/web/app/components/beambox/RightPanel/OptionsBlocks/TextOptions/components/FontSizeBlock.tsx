  ...
  if (isTablet) {
    return (
      <ControlBlock label={t.font_size} type={ControlType.FONT_SIZE}>
        <InputNumberGroup config={config} onChange={(e) => handleChange(Number(e))} value={state.value} />
      </ControlBlock>
    );
  }

  return (
    <ControlBlock className={styles.container} title={t.font_size} type={ControlType.FONT_SIZE}>
      <OptionsInput
