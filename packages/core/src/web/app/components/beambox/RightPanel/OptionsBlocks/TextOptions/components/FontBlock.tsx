...
const FontBlock = ({
  ...
  return isTablet ? (
    <Row>
      <ControlBlock className={styles.family} label={langOptionPanel.font_family} type={ControlType.FONT_FAMILY}>
        {familySelect}
      </ControlBlock>
      <ControlBlock className={styles.style} label={langOptionPanel.font_style} type={ControlType.FONT_STYLE}>
        {styleSelect}
      </ControlBlock>
    </Row>
  ) : (
    <>
      ...
      <div className={styles['labeled-row']}>
        <span className={`${styles['prefix-label']} ${styles.bold}`}>B</span>
        {styleSelect}
      </div>
    </>
  );
