...
const useMenuData = (email?: string): MenuNode[] => {
  ...
  const fileMenu: MenuNode = {
    children: [
      ...
      ...(isTemplateMode ? [{ id: 'SAVE_AS_PROJECT', label: mockT('save_as_project'), type: 'item' } as const] : []),
      ...(isProjectMode ? [{ id: 'SAVE_AS_TEMPLATE', label: mockT('save_as_template'), type: 'item' } as const] : []),
