...
const mockContextState = { pulseWidth: { hasMultiValue: false, value: 40 } };
...
describe('test PulseWidthBlock', () => {
  ...
  it('should render correctly when type is panel-item', () => {
    const { container } = render(
      ...
      >
        <PulseWidthBlock max={350} min={2} type="panel-item" />
      </ConfigPanelContext.Provider>,
    );
  ...
  test('onChange should work', () => {
    ...
    expect(mockDispatch).toHaveBeenLastCalledWith({
      payload: { configName: 'CUSTOM_PRESET_CONSTANT', pulseWidth: 88 },
      type: 'change',
    });
  ...
  test('onChange should work correctly when type is modal', () => {
    const { container } = render(
      ...
      >
        <PulseWidthBlock max={350} min={2} type="modal" />
      </ConfigPanelContext.Provider>,
    );
