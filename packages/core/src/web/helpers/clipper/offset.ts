...
const UNSUPPORTED_TAGS = ['g', 'image', 'text', 'use'];
...
async function processElementForOffset(
  ...
  try {
    ...
    await match({ cornerType, isPathClosed })
      ...
      .with(
        { isPathClosed: true },
        async () => await clipperInstance.addPaths(paths, ClipperLib.JoinType.jtMiter, ClipperLib.EndType.etClosedLine),
      )
      .otherwise(
        async () => await clipperInstance.addPaths(paths, ClipperLib.JoinType.jtMiter, ClipperLib.EndType.etOpenSquare),
      );
  ...
  for (const path of scaledPaths) {
    ...
    if (!simplify) {
      ...
      const points = path.map(({ X, Y }) => ({
        x: Math.floor(100 * (X / SCALE_FACTOR)) / 100, // Round to 2 decimal places
...
const offsetElements = async (
  ...
  if (dir === 1 && solutionPaths.length > 0) {
    // Outward
  ...
  const newElem = svgCanvas.addSvgElementFromJson({
    attr: {
      d: pathD,
      fill: beamboxPreference.read('vector_fill_removed_with_offset')
        ? 'none'
        : targetElements[0]?.getAttribute('fill') || '#000', // Example: inherit fill or use default
      'fill-opacity': beamboxPreference.read('vector_fill_removed_with_offset')
        ? 0
        : targetElements[0]?.getAttribute('fill-opacity') || 1,
      id: svgCanvas.getNextId(),
      stroke: targetElements[0]?.getAttribute('stroke') || '#000', // Example: inherit stroke
      'stroke-width': targetElements[0]?.getAttribute('stroke-width') || '1', // Example: inherit stroke-width
    },
    element: 'path',
  });
