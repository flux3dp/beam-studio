...
const getDpmm = async (arrayBuffer: ArrayBuffer, fileType: string): Promise<number[]> => {
  if (fileType === 'image/png') {
    ...
    while (offset < bytes.length) {
      ...
      if (chunkType === 'pHYs') {
        ppmX = (bytes[offset + 8] << 24) | (bytes[offset + 9] << 16) | (bytes[offset + 10] << 8) | bytes[offset + 11];
        ppmY = (bytes[offset + 12] << 24) | (bytes[offset + 13] << 16) | (bytes[offset + 14] << 8) | bytes[offset + 15];
        pHYsFound = true;
        break;
      }
