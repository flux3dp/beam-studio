...
const chordLengthParameterize = (points: vector2D[], start: number, end: number) => {
  ...
  const u = Array(end - start + 1) as number[];
...
const generateBezier = (
  ...
  const numPoints = end - start + 1;
  const A = Array(numPoints) as vector2D[][];
  ...
  for (let i = 0; i < numPoints; i += 1) {
    ...
    let tmp = v2Sum(
      v2Scale(points[start], B0(u[i])),
      v2Scale(points[start], B1(u[i])),
      v2Scale(points[end], B2(u[i])),
      v2Scale(points[end], B3(u[i])),
    );
...
const reparameterize = (
  ...
  curveControlPoints: vector2D[],
  u: number[],
) => {
  const numPoints = end - start + 1;
  const uPrime = Array(numPoints) as number[];
...
const NewtonRaphsonRootFind = (curveControlPoints: vector2D[], point: vector2D, currentU: number) => {
  const Q1 = Array.from({ length: 3 }) as vector2D[];
  const Q2 = Array.from({ length: 2 }) as vector2D[];
  ...
  const denominator =
    point1U.x ** 2 + point1U.y ** 2 + (pointU.x - point.x) * point2U.x + (pointU.y - point.y) * point2U.y;
...
const bezierCurveValue = (degree: number, curveControlPoints: vector2D[], t: number) => {
  const tempControlPoints = Array(degree + 1) as vector2D[];
