  ...
  try {
    ...
    if (!success) {
      alertCaller.popUpError({
        message: mockT('This file is not supported'),
      });
    }
  }
};
...
export const exportContents = async (ownerId: string) => {
  ...
  const defaultFileName = mockT('LibraryContents');

  await dialog.writeFileDialog(getContent, mockT('Save library contents'), defaultFileName, [
...
export const addContentFromDialog = async (owner: SVGElement) => {
  ...
  try {
    ...
    if (isUse) {
      ...
      if (!success) {
        alertCaller.popUpError({
          message: mockT('This svg is not supported'),
        });
      }
    } else {
