...
type FileType = 'Example Files' | 'Material Test';

function checkExample(exampleName: string, res: string | undefined, type?: FileType) {
  cy.get('div.menu-btn-container').click();
  cy.get('.rc-menu__submenu').contains('File').click();
  cy.get('.rc-menu__submenu').contains('Examples').click();
  if (type) cy.get('.rc-menu__submenu').contains(type).click();
...
const exampleList: { label: string; type?: FileType; adorRes?: string; adorResWin?: string; beamseriesRes?: string }[] =
  [
    {
...
describe('example', () => {
  ...
  exampleList.forEach(({ label, adorRes, adorResWin, beamseriesRes, type }) => {
    it(`import ${label} with ador workarea`, () => {
      cy.changeWorkarea('Ador');
      checkExample(label, (isWindows ? adorResWin : adorRes) ?? adorRes, type);
    });

    it(`import ${label} with beamseries workarea`, () => {
      checkExample(label, beamseriesRes, type);
    });
  });
});
